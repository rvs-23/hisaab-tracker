...
row = compute.budget_series(active, d.income).query("year == @year")
investment = float(row.iloc[0]["investment"]) if not row.empty else 0.0

section(f"Investment to deploy · {year}")
mc = st.columns(2)
metric_tile(mc[0], "This year", inr_short(investment), "to invest", big=True)
metric_tile(mc[1], "Per month", inr_short(investment / 12), "to invest", big=True)
...
base = f"alloc_{active.key}_{year}"
